...
# Local imports
from spyder_e import __trouble_url__
from spyder_e.api.utils import get_class_values
from spyder_e.api.widgets.auxiliary_widgets import SpyderWindowWidget
from spyder_e.api.plugins import Plugins
from spyder_e.app.tests.conftest import (
...
from spyder_e.config.base import (
    get_home_dir, get_conf_path, get_module_path, running_in_ci,
    running_in_ci_with_conda)
from spyder_e.config.manager import CONF
from spyder_e.dependencies import DEPENDENCIES
from spyder_e.plugins.debugger.api import DebuggerWidgetActions
from spyder_e.plugins.externalterminal.api import ExtTerminalShConfiguration
from spyder_e.plugins.help.widgets import ObjectComboBox
from spyder_e.plugins.help.tests.test_plugin import check_text
from spyder_e.plugins.ipythonconsole.utils.kernel_handler import KernelHandler
from spyder_e.plugins.ipythonconsole.api import (
    IPythonConsolePyConfiguration,
    IPythonConsoleWidgetMenus
)
from spyder_e.plugins.mainmenu.api import ApplicationMenus
from spyder_e.plugins.layout.layouts import DefaultLayouts
from spyder_e.plugins.profiler.widgets.main_widget import ProfilerWidgetActions
from spyder_e.plugins.toolbar.api import ApplicationToolbars
from spyder_e.plugins.run.api import (
...
from spyder_e.plugins.shortcuts.widgets.table import SEQUENCE
from spyder_e.utils.conda import get_list_conda_envs
from spyder_e.utils.misc import remove_backslashes, rename_file
from spyder_e.utils.clipboard_helper import CLIPBOARD_HELPER
from spyder_e.utils.programs import find_program
from spyder_e.utils.qthelpers import qbytearray_to_str
from spyder_e.widgets.dock import DockTitleBar
...
def test_single_instance_and_edit_magic(main_window, qtbot, tmpdir):
    ...
    spy_dir = osp.dirname(get_module_path('spyder_e'))
    lock_code = (
        "import sys\n"
        "sys.path.append(r'{spy_dir_str}')\n"
        "from spyder_e.utils.external import lockfile\n"
        "lock_file = r'{lock_file}'\n"
        "lock = lockfile.FilesystemLock(lock_file)\n"
        "lock_created = lock.lock()\n"
        "print(lock_created)".format(
            spy_dir_str=spy_dir,
            lock_file=get_conf_path('spyder_e.lock'))
    )
...
def test_project_path(main_window, tmpdir, qtbot):
    ...
    # Ensure that project path is removed from spyder_e_pythonpath
    assert path not in projects.get_conf(
        'spyder_pythonpath', section='pythonpath_manager')
...
def test_plots_plugin(main_window, qtbot, tmpdir, mocker):
    ...
    mocker.patch('spyder_e.plugins.plots.widgets.figurebrowser.getsavefilename',
...
def test_plot_from_collectioneditor(main_window, qtbot):
    ...
    from spyder_e.widgets.collectionseditor import CollectionsEditor
...
def test_run_static_code_analysis(main_window, qtbot):
    ...
    from spyder_e.plugins.pylint.main_widget import PylintWidgetActions
...
def test_troubleshooting_menu_item_and_url(main_window, qtbot, monkeypatch):
    ...
    attr_to_patch = ('spyder_e.utils.qthelpers.QDesktopServices')
...
def test_print_comms(main_window, qtbot):
    ...
    # Create some output from spyder_e call
    with qtbot.waitSignal(shell.executed):
        shell.execute(code)
    ...
    assert ("Output from spyder_e call" in control.toPlainText())
