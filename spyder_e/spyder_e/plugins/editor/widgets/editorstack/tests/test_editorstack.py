...
# Local imports
from spyder_e.config.base import get_conf_path, running_in_ci
...
def visible_editor_bot(editor_bot, mocker):
    ...
    # message box.
    mocker.patch(
        'spyder_e.plugins.editor.widgets.editorstack.editorstack.osp.isfile',
        returned_value=True
    )
...
def test_opening_sets_file_hash(base_editor_bot, mocker):
    ...
    editor_stack = base_editor_bot
    mocker.patch('spyder_e.plugins.editor.widgets.editorstack.editorstack.encoding.read',
...
def test_maybe_autosave_handles_error(editor_bot, mocker):
    ...
    mock_dialog = mocker.patch(
        'spyder_e.plugins.editor.utils.autosave.AutosaveErrorDialog')
    try:
