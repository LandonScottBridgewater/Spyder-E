...
# Local imports
from spyder_e.plugins.editor.widgets.recover import (make_temporary_files,
                                                   RecoveryDialog)
...
def test_recoverydialog_restore_when_original_not_recorded(
    ...
    mocker.patch('spyder_e.plugins.editor.widgets.recover.getsavefilename',
...
def test_recoverydialog_restore_fallback(qtbot, recovery_env, mocker):
    ...
    """
    orig_dir, autosave_dir, autosave_mapping = recovery_env
    mocker.patch('spyder_e.plugins.editor.widgets.recover.os.replace',
                 side_effect=OSError)
    dialog = RecoveryDialog(autosave_mapping)
    table = dialog.findChild(QTableWidget)
    button = table.cellWidget(0, 2).findChildren(QPushButton)[0]
...
def test_recoverydialog_restore_when_error(qtbot, recovery_env, mocker):
    ...
    mocker.patch('spyder_e.plugins.editor.widgets.recover.os.replace',
                 side_effect=OSError)
    mocker.patch('spyder_e.plugins.editor.widgets.recover.shutil.copy2',
                 side_effect=IOError)
    mock_QMessageBox = mocker.patch(
                'spyder_e.plugins.editor.widgets.recover.QMessageBox')
    dialog = RecoveryDialog(autosave_mapping)
    table = dialog.findChild(QTableWidget)
    button = table.cellWidget(0, 2).findChildren(QPushButton)[0]
    button.click()
...
def test_recoverydialog_discard_when_error(qtbot, recovery_env, mocker):
    ...
    """
    orig_dir, autosave_dir, autosave_mapping = recovery_env
    mocker.patch('spyder_e.plugins.editor.widgets.recover.os.remove',
                 side_effect=OSError)
    mock_QMessageBox = mocker.patch(
                'spyder_e.plugins.editor.widgets.recover.QMessageBox')
    dialog = RecoveryDialog(autosave_mapping)
