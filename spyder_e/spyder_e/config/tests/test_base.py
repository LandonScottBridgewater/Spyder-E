...
"""
Tests for the spyder_e.config.base module.
"""
...
def test_is_stable_version():
    ...
    for stable_version in ['3.3.0', '2', ('0', '5')]:
        assert spyder_e.config.base.is_stable_version(stable_version)
    for not_stable_version in ['4.0.0b1', '3.3.2.dev0',
                               'beta', ('2', '0', 'alpha')]:
        assert not spyder_e.config.base.is_stable_version(not_stable_version)
...
def test_get_conf_path(monkeypatch, use_dev_config_dir):
    ...
    reload(spyder_e.config.base)
    conf_path = spyder_e.config.base.get_conf_path()
    assert conf_path
...
@pytest.mark.skipif(
    not spyder_e.config.base.running_in_ci(), reason="Only works on CIs"
)
@pytest.mark.skipif(
...
def test_is_conda_based_app():
    ...
    assert spyder_e.config.base.is_conda_based_app(pyexec=pyexec)
