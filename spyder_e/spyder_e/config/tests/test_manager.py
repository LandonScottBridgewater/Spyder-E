...
# Local imports
from spyder_e.config.base import get_conf_path, get_conf_paths, running_in_ci
from spyder_e.config.manager import ConfigurationManager
...
def test_site_config_load():
    ...
    for i, path in enumerate(reversed(get_conf_paths())):
        ...
        conf_fpath = os.path.join(path, 'spyder_e.ini')
...
startup/run_lines =
"""
    conf_fpath = get_conf_path('spyder_e.ini')
    ...
    with open(osp.join(user_path, 'spyder_e.ini'), 'r') as f:
        user_contents = f.read()

    plugin_path = manager.get_plugin_config_path('internal_console')
    with open(osp.join(plugin_path, 'spyder_e.ini'), 'r') as f:
