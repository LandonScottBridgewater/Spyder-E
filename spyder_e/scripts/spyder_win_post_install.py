...
def install():
    ...
    script = osp.abspath(osp.join(sys.prefix, 'scripts', 'spyder_e'))
    if not osp.exists(script): # if not installed to the site scripts dir
        script = osp.abspath(osp.join(osp.dirname(osp.abspath(__file__)), 'spyder_e'))
    workdir = "%HOMEDRIVE%%HOMEPATH%"
    lib_dir = sysconfig.get_path("platlib")
    ico_dir = osp.join(lib_dir, 'spyder_e', 'windows')
