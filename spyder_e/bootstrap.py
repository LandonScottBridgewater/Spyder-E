...
parser.add_argument('--filter-log', default='',
                    help="Comma-separated module name hierarchies whose log "
                         ...
                         "spyder_e.plugins.completion,spyder_e.plugins.editor")
...
if args.debug:
    ...
    if "spyder_e.config.base" in sys.modules:
...
logger.info("Imported Spyder %s - Revision %s, Branch: %s; "
            ...
            versions['spyder_e'], versions['revision'], versions['branch'],
...
if args.safe_mode or os.environ.get('SPYDER_SAFE_MODE'):
    from spyder_e.config.base import get_conf_path  # analysis:ignore
    conf_dir = Path(get_conf_path())
    if conf_dir.is_dir():
        shutil.rmtree(conf_dir)

logger.info("Running Spyder")
from spyder_e.app import start  # analysis:ignore
