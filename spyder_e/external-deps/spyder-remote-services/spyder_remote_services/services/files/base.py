...
from spyder_e_remote_services.services.files.compression import ZipStream, MemberFile, CompressionType
...
class FileWebSocketHandler(WebSocketHandler):
    ...
    def atomic_path(self):
        ...
        return self.path.parent / f".{self.path.name}.spyder_e.tmp"

    @property
    def lock_path(self):
        """Get the path to the atomic file."""
        return self.path.parent / f".{self.path.name}.spyder_e.lck"
